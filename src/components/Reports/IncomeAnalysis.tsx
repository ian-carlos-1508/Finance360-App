/* Replace file: src/components/Analytics/IncomeAnalysis.tsx */
...
// --- Type Definitions ---
type IncomeRecord = AllTransaction & { main_category: string; account_name: string };
...
// --- Constants & Colors (Income Themed) ---
// --- REMOVED old PIE_COLORS and LINE_COLORS constants ---
...
type CustomMultiSelectProps = {
  ...
  dropdownRef: React.RefObject<HTMLDivElement | null>; // Allows null
};
...
function IncomeAnalysis() {
  ...
  const [pieTransactions, setPieTransactions] = useState<IncomeRecord[]>([]);
  ...
  useEffect(() => {
    const fetchFilterData = async () => {
      ...
      if (accData) {
        // CRITICAL FIX: Filter out credit cards from the selectable list
        const nonCreditCards = accData.filter(
          (acc) => acc.type !== 'credit_card'
        );
        setAccountList(nonCreditCards as Account[]);
      }
    };
    fetchFilterData();
  }, []);
  ...
  useEffect(() => {
    const fetchPieData = async () => {
      ...
      // Assumes 'get_income_report' SQL function properly filters out credit cards
      ...
      if (error) {
        ...
        setPieTransactions(data as IncomeRecord[]);
      }
  ...
  // --- Data Fetching & Processing: Progression (Section 2 - LOGIC CHANGED) ---
  useEffect(() => {
    const fetchProgressionData = async () => {
      ...
      const params: any = {
        ...
        // The series (lines) are NOW ALWAYS 'account'.
        p_series_group: 'account',
      };

      // The category dropdown now acts as a FILTER, not a series selector.
      if (progCategory !== 'all') {
        params.p_main_category = progCategory;
      }

      // Call the NEW function 'get_income_progression_series'
      const { data, error } = await supabase.rpc(
        'get_income_progression_series',
        params
      );
  ...
  useEffect(() => {
    const fetchComparisonData = async () => {
      ...
      // Assumes 'get_period_comparison' SQL function properly filters out credit cards
      const { data, error } = await supabase.rpc(
  ...
  const kpiTitle = selectedMainCategory
    ? `Analysis for ${selectedMainCategory} Income`
    : 'Analysis for All Income';

  // For Income: Positive growth is good (green)
  ...
  const barXAxisFormatter = (value: number) => formatCurrency(value);
  ...
  return (
    <div>
      ...
      {/* --- SECTION 1: Pie Charts (Income Themed) --- */}
      ...
      >
        <div className={`${styles.kpiCard} ${styles.green}`}>
          <h3 className={styles.kpiTitle}>Total Income</h3>
          ...
          >
            ...
            >
              {selectedMainCategory
                ? `Income by Account (for ${selectedMainCategory})`
                : 'Income by Account'}
            </h2>
            <ChartToggle chartType={chartType} setChartType={setChartType} />
          </div>
      ...
      {/* --- SECTION 2: Income Progression (VISUALS & LOGIC FIXED) --- */}
      <div
        className={`${styles.card} ${styles.tableSection}`}
        style={{ marginTop: '2rem' }}
      >
        <h2 className={styles.cardTitle}>Income Progression by Account</h2>
        <div
          className={styles.filterWrapper}
          style={{ justifyContent: 'flex-start' }}
        >
        ...
        >
          {loadingProgression ? (
            ...
            <p style={{ textAlign: 'center', padding: '1rem' }}>
              No income data available for this period/filter.
            </p>
          )}
        </div>
      </div>
